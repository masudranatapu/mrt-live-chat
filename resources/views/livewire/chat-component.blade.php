    ...
    <div style="overscroll-behavior: none;">
        ...
        <div class="mt-20 mb-16">
            @foreach ($messages as $value)
                @if ($value['sender'] != auth()->user()->name)
                    <div class="clearfix">
                        <div class="bg-gray-300 w-2/4 mx-4 my-3 p-2 rounded-lg">
                            <strong>{{ $value['receiver'] }} :</strong> {{ $value['message'] }}
                        </div>
                    </div>
                @else
                    <div class="clearfix">
                        <div class="text-right">
                            <div class="bg-green-300 float-right w-2/4 mx-4 my-4 p-2 rounded-lg clearfix">
                                <strong>{{ $value['sender'] }} :</strong> {{ $value['message'] }}
                            </div>
                        </div>
                    </div>
                @endif
    ...
    <form wire:submit="sendChatMessage()">
        <div class="fixed w-full flex justify-between bg-green-100" style="bottom: 0px;">
            <textarea class="flex-grow m-2 py-2 px-4 mr-1 rounded-full border border-gray-300 bg-gray-200 resize-none"
                wire:model="message" rows="1" placeholder="Message..." style="outline: none;">
            </textarea>
